...
def get_data(job, page):
    ...
    data_list = res2.json()['resultbody']['job']['items']
    for data in data_list:
        ...
        date_list.append([jobName.strip(), jobTags.strip(), jobAreaString.strip(), jobDescribe.strip(), fullCompanyName.strip(), degreeString.strip(), companySizeString.strip(), companyTypeString.strip(), industryType2Str.strip(), provideSalaryString.strip()])
    # 保存数据到CSV文件
    df = pd.DataFrame(date_list, columns=["职业名称", "工作要求", "工作地点", "工作声明", "公司名称", "学历要求", "企业规模", "企业性质", "企业领域", "工资"])

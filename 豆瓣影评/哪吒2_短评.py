...
for page in range(0, 15):
    ...
    response = requests.get('https://movie.douban.com/subject/34780991/reviews', params=params, cookies=cookies, headers=headers)
    ...
    score_list = tree.xpath("/html/body/div/div/div/div/div/div/div/header/span/@title")
    duan_pin_list = tree.xpath("//div[@class='main-bd']/h2/a/text()")
    chang_pin_list = tree.xpath("//div[@class='main-bd']/div/div[@class='short-content']/text()")

    chang_pin_li = [chang_pin.replace('\n', '').strip() for chang_pin in chang_pin_list if chang_pin.strip()]

    # 确保所有列表长度一致
    min_length = min(len(title_list), len(time_list), len(score_list), len(duan_pin_list), len(chang_pin_li))
    print(min_length)
    title_list = title_list[:min_length]
    time_list = time_list[:min_length]
    score_list = score_list[:min_length]
    duan_pin_list = duan_pin_list[:min_length]
    chang_pin_li = chang_pin_li[:min_length]

    for title, times, score, duan_pin, chang_pin in zip(title_list, time_list, score_list, duan_pin_list, chang_pin_li):
        data.append([title, times, score, duan_pin])
        print(title, times, score, duan_pin)

# 将数据保存到 DataFrame
df = pd.DataFrame(data, columns=['user', 'Time', 'Score', 'Short Pin'])
